#' Method to transform htest object into a flextable object
#'
#' @description
#' This method tabularises an htest object as a flextable.
#'
...
#' @param show.signif.stars If TRUE add the sifnificance stars to the table
...
#' tabularise(cor)
#'
#' tabularise(t.test(1:10, y = c(7:20)), lang = "fr")
#'
tabularise_default.htest <- function(data,
  header = TRUE,
  ...
  if (length(res$estimate) > 1) {
    if (x$method %in% c("Welch Two Sample t-test", " Two Sample t-test")) {
      ...
      names(res$estimate) <- "difference mean"
    } else {
  ...
  if (!is.null(x$estimate)) {
    ...
    if(!is.null(names(res$estimate))) {
      ...
      esti[esti %in% names(info_lang[["estimate"]])] <- info_lang[["estimate"]][esti]
  ...
  if (!is.null(x$statistic)) {
    ...
    statistic[statistic %in% names(info_lang[["statistic"]])] <- info_lang[["statistic"]][statistic]
  } else {
    statistic <- "Statistic"
  }
  ...
  if (!is.null(x$parameter)) {
    ...
    parameter[parameter %in% names(info_lang[["parameter"]])] <- info_lang[["parameter"]][parameter]
  } else {
    parameter <- "Parameter"
  }
  ...
  if (!is.null(x$conf.int)) {
    ft <- mk_par(ft, j="estimate", value = as_paragraph(
      estimate, " (",x$conf.int[[1]], "-", x$conf.int[[2]], ")")
    )

    ft <- mk_par(ft, j="estimate",part = "header", value = para_md(paste0(
...
infos_en_htest <- list(
  ...
  method = list(
    " Two Sample t-test" = c(
      "two.sided" = "Two sided independent samples t-test", # "Two sided two-sample t-test"
    ...
    "Wilcoxon signed rank test with continuity correction" = c(
      "two.sided" = "Two sided Wilcoxon signed rank test with continuity correction",
      "less" = "Left sided Wilcoxon signed rank test with continuity correction",
      "greater" = "Right sided Wilcoxon signed rank test with continuity correction"
    ),
    ...
    "Chi-squared test for given probabilities" = "$\\chi^{2}$ test for given probabilities",
    "Pearson's Chi-squared test with Yates' continuity correction" = "Pearson's $\\chi^{2}$ test with Yates' continuity correction"
  ),
  statistic = c(
    "t" = "*t*~obs.~ value",
    "W" = "*W*~obs.~ value",
  ...
  estimate = c(
    "difference mean" = "Difference of means",
    "mean difference" = "Average of the means",
...
infos_fr_htest <- list(
  ...
  method = list(
    " Two Sample t-test" = c(
      "two.sided" = "Test t bilat\u00e9ral d'ind\u00e9pendance  avec variances \u00e9gales",
      "less" = "Test t unilat\u00e9ral \u00e0 gauche d'ind\u00e9pendance avec variances \u00e9gales",
      "greater" = "Test t unilat\u00e9ral \u00e0 droite d'ind\u00e9pendance avec variances \u00e9gales"),
    "Welch Two Sample t-test" = c(
      "two.sided" = "Test bilat\u00e9ral d'ind\u00e9pendance de Welch",
      "less" = "Test unilat\u00e9ral \u00e0 gauche d'ind\u00e9pendance de Welch",
      "greater" = "Test unilat\u00e9ral \u00e0 droite d'ind\u00e9pendance de Welch"
    ),
    "Paired t-test" = c(
    ...
    "Pearson's product-moment correlation" = c(
      ...
      "less" = "Test de corr\u00e9lation de Pearson unilat\u00e9ral \u00e0 gauche",
      "greater" = "Test de corr\u00e9lation de Pearson unilat\u00e9ral \u00e0 droite"
    ),
    "Kendall's rank correlation tau" = c(
      "two.sided" = "Test de corr\u00e9lation de Kendall bilat\u00e9ral",
      "less" = "Test de corr\u00e9lation de Kendall unilat\u00e9ral \u00e0 gauche",
      "greater" = "Test de corr\u00e9lation de Kendall unilat\u00e9ral \u00e0 droite"
    ),
    "Spearman's rank correlation rho" = c(
      "two.sided" = "Test de corr\u00e9lation de Spearman bilat\u00e9ral",
      "less" = "Test de corr\u00e9lation de Spearman unilat\u00e9ral \u00e0 gauche",
      "greater" = "Test de corr\u00e9lation de Spearman unilat\u00e9ral \u00e0 droite"
    ),
    "Kruskal-Wallis rank sum test" = "Test de Kruskal-Wallis",
    "Pearson's Chi-squared test" = "Test de $\\chi^{2}$ d'ind\u00e9pendance",
    "Chi-squared test for given probabilities" = "Test de $\\chi^{2}$ pour des probabilit\u00e9s donn\u00e9es",
    "Pearson's Chi-squared test with Yates' continuity correction" = "Test de $\\chi^{2}$ d'ind\u00e9pendance avec correction de Yates",
    "Bartlett test of homogeneity of variances" = "Test d'homog\u00e9n\u00e9it\u00e9 des variances de Bartlett"
  ),
  estimate = c(
    "difference mean" = "Diff\u00e9rence des moyennes",
